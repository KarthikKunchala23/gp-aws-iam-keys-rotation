...
Single AWS Lambda entrypoint used for both scheduled rotation and API Gateway confirmation.
...
The handler dispatches based on the API Gateway HTTP API route:

```text
POST /rotations/{rotation_id}/confirm
```

For the confirmation route it:

1. Reads `pathParameters.rotation_id`
2. Calls `confirm_handoff(rotation_id)`
3. Verifies the DynamoDB record is `HANDOFF_PENDING`
4. Changes the state to `HANDOFF_CONFIRMED`
5. Records `confirmation_received_at`
6. Returns an HTTP response to API Gateway

For scheduled invocations, the handler falls through to the normal rotation workflow:
...
The actual business logic remains in the reusable rotation engine.

---

## `lambda/lambda_function.py`

This file is retained in the repository as an earlier/legacy confirmation-handler implementation.

It is **not deployed as a separate Lambda** in the current architecture.

The current implementation intentionally uses a **single Lambda function** (`rotation_iam_keys`) for both:

* EventBridge Scheduler-based rotation
* API Gateway developer confirmation
...
Provides the HTTPS confirmation endpoint used by developers to confirm that an application has been updated to use the replacement access key.
...
```text
API Name: iam-key-rotation-confirmation
API ID: nxrraikebk
Protocol: HTTP
```
...
```text
https://nxrraikebk.execute-api.ap-south-1.amazonaws.com
```

Example endpoint:

```text
https://nxrraikebk.execute-api.ap-south-1.amazonaws.com/rotations/{rotation_id}/confirm
```

The `rotation_id` is generated by the rotation workflow:

```text
user_name#old_access_key_id#new_access_key_id
```

Because `#` is a reserved URL fragment character, it must be URL-encoded as `%23` when constructing the HTTPS request.

Example:

```bash
curl -i -X POST   "https://nxrraikebk.execute-api.ap-south-1.amazonaws.com/rotations/terraform-execution%23OLD_KEY_ID%23NEW_KEY_ID/confirm"
```
...
API Gateway uses an AWS_PROXY integration with the existing `rotation_iam_keys` Lambda.
...
routeKey detection
    │
    ▼
confirm_handoff(rotation_id)
    │
    ▼
DynamoDB
```

The Lambda detects the API Gateway route using:
...
```python
rotation_id = event["pathParameters"]["rotation_id"]
```
...
The API does not deactivate the old key immediately.
...
The scheduled rotation process later evaluates the observation period and performs the deactivation when the configured safety window has elapsed.
...
Already-confirmed or otherwise invalid state:

```text
HTTP 409
```

For example, attempting to confirm a rotation that is already `HANDOFF_CONFIRMED` returns a conflict rather than performing the transition again.

### API Gateway CLI Verification
...
```bash
aws apigatewayv2 get-routes   --api-id nxrraikebk   --region ap-south-1
```

Inspect the confirmation integration:

```bash
aws apigatewayv2 get-integration   --api-id nxrraikebk   --integration-id djxb0om   --region ap-south-1
```

The integration is expected to target:
...
```bash
curl -i -X POST   "https://nxrraikebk.execute-api.ap-south-1.amazonaws.com/rotations/<URL_ENCODED_ROTATION_ID>/confirm"
```
...
A **single Lambda function** is used for both execution paths:
...
EventBridge Scheduler invokes:

```text
rotation_runner.lambda_handler
```

with the normal scheduled event.
...
API Gateway invokes the **same Lambda** using an AWS_PROXY integration.
...
This avoids maintaining a second confirmation Lambda and keeps the rotation state transition in the same deployed application.
...
# API Gateway Integration IAM

API Gateway uses an AWS_PROXY Lambda integration to invoke:

```text
arn:aws:lambda:ap-south-1:897722700244:function:rotation_iam_keys
```

The confirmation integration is:

```text
Integration ID: djxb0om
Type: AWS_PROXY
Method: POST
Payload Format: 2.0
```
...
is connected directly to the existing `rotation_iam_keys` Lambda.

The Lambda resource policy must allow API Gateway to invoke the function.

This is separate from the EventBridge Scheduler IAM role:
...
The current EventBridge Scheduler is:
...
The developer confirmation path is independent of the daily trigger:
...
The next scheduled execution then evaluates the confirmed rotation and continues the state machine.

---

# API Gateway + EventBridge Integration Summary

The two external triggers intentionally invoke the same Lambda through different paths:
...
```text
                     ...
                     │                                 │
                     ▼                                 ▼
             Scheduled path                    Confirmation path
                     │                                 │
             EventBridge Scheduler                API Gateway
                     │                                 │
              Daily at 18:00              POST /rotations/{rotation_id}/confirm
                     │                                 │
                     ▼                                 ▼
               run_rotation()                   confirm_handoff()
                     │                                 │
                     └────────────────┬────────────────┘
                                      ▼
                                  DynamoDB
...
* **EventBridge Scheduler** drives the unattended daily lifecycle.
* **API Gateway** provides the developer-facing HTTPS confirmation interface.
* **Both paths use the same Lambda deployment.**
* **DynamoDB remains the source of truth for rotation state.**
* API confirmation only advances the state to `HANDOFF_CONFIRMED`; it does not immediately deactivate the old key.
